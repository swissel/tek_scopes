...
scope3 = tektronix(addr='169.254.62.22') # LAN ethernet address of scope with switch
...
print "Writing %d events on %d channels to file %s"%(num_events, num_chan, finame)
with open(finame,'a') as csvfile:
	...
	while 1 == scope3.acq_state:
	    pass
	print "Event number "+format(i+1)+" Time: "+datetime.now().strftime("%H:%M:%S.%f")
	scope3.acq_state=0
	for j in range(num_chan):
	    ...
	    #print "Data: "+format(data)
	    writer.writerow([j+1,i+1,datetime.now().strftime("%H:%M:%S.%f"),x_inc,x_zero,','.join(map(str,data))])
